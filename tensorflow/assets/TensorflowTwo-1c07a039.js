 ...
 * limitations under the License.
 * =============================================================================
 */function Ai(a,e){const t=b(a,"x","reverse");return S(t.rank===4,()=>`Error in reverse4D: x must be rank 4 but got rank ${t.rank}.`),le(t,e)}const ki=v({reverse4d_:Ai});/**
 * @license
 ...
 * limitations under the License.
 * =============================================================================
 */async function xi(a,e,t){const s=b(a,"tensor","boolMask"),r=b(e,"mask","boolMask","bool"),i=t??0,o=r.rank,u=s.shape;S(o>0,()=>"mask cannot be scalar"),_t(u.slice(i,i+o),r.shape,"mask's shape must match the first K dimensions of tensor's shape,");let p=1;for(let y=i;y<i+o;y++)p*=u[y];const m=u.slice(0,i).concat([p],u.slice(i+o)),l=A(s,m),c=A(r,[-1]),d=await jt(c),f=$t(d,[1]),g=Ct(l,f,i);return a!==s&&s.dispose(),e!==r&&r.dispose(),f.dispose(),l.dispose(),c.dispose(),d.dispose(),g}const Ri=xi;/**
 * @license
 * Copyright 2018 Google LLC. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 ...
 * limitations under the License.
 * =============================================================================
 */function Qi({x:a,filter:e,strides:t,pad:s,dataFormat:r="NHWC",dilations:i=[1,1],dimRoundingMode:o,bias:u,activation:p="linear",preluActivationWeights:m,leakyreluAlpha:l}){if(ba(D.state.gradientDepth,p)===!1){let I=zt(a,e,t,s,r,i,o);return u!=null&&(I=ae(I,u)),Ta(I,p,m,l)}const c=b(a,"x","depthwiseConv2d","float32"),d=b(e,"filter","depthwiseConv2d","float32");let f=c,g=!1;c.rank===3&&(g=!0,f=A(c,[1,c.shape[0],c.shape[1],c.shape[2]])),S(f.rank===4,()=>`Error in fused depthwiseConv2d: input must be rank 4, but got rank ${f.rank}.`),S(d.rank===4,()=>`Error in fused depthwiseConv2d: filter must be rank 4, but got rank ${d.rank}.`),S(f.shape[3]===d.shape[2],()=>`Error in fused depthwiseConv2d: number of input channels (${f.shape[3]}) must match the inChannels dimension in filter ${d.shape[2]}.`),i==null&&(i=[1,1]),S(wa(t,i),()=>`Error in fused depthwiseConv2d: Either strides or dilations must be 1. Got strides ${t} and dilations '${i}'`),Sa("fused depthwiseConv2d",s,o);const y=_a(f.shape,d.shape,t,i,s,o,!0);let h;u!=null&&(h=b(u,"bias","fused conv2d"),[h]=va(h,c),Oa(y.outShape,h.shape));let N;m!=null&&(N=b(m,"prelu weights","fused depthwiseConv2d"));const _=(I,$)=>{S(Ea(i),()=>`Error in gradient of fused depthwiseConv2d: dilation rates greater than 1 are not yet supported. Got dilations '${i}'`);const[w,C,O,E]=$,V=Ia(I,O,p),H=ms(C.shape,V,w,t,s,i,o),ne=ls(C,V,w.shape,t,s,i,o);if(E!=null){const te=Aa(h,V);return[H,ne,te]}return[H,ne]},k={x:f,filter:d,bias:h,preluActivationWeights:N},T={strides:t,pad:s,dataFormat:r,dilations:i,dimRoundingMode:o,activation:p,leakyreluAlpha:l};return u==null?nt(($,w,C)=>{let O=D.runKernel(it,k,T);return C([w,$,O]),g&&(O=A(O,[O.shape[1],O.shape[2],O.shape[3]])),{value:O,gradFunc:_}})(f,d):nt(($,w,C,O)=>{let E=D.runKernel(it,k,T);return O([w,$,E,C]),g&&(E=A(E,[E.shape[1],E.shape[2],E.shape[3]])),{value:E,gradFunc:_}})(f,d,h)}const Mi=v({fusedDepthwiseConv2d_:Qi});/**
 * @license
 * Copyright 2019 Google LLC. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 ...
 */class pu{constructor(e,t,s,r,i,o,u){this.name=e,this.dtype=t,this.maxSize=s,this.elementShape=r,this.identicalElementShapes=i,this.dynamicSize=o,this.clearAfterRead=u,this.tensors=[],this.closed_=!1,this.idTensor=K(0),G(this.idTensor)}get id(){return this.idTensor.id}get closed(){return this.closed_}clearAndClose(e){this.tensors.forEach(t=>{(e==null||!e.has(t.tensor.id))&&t.tensor.dispose()}),this.tensors=[],this.closed_=!0,this.idTensor.dispose()}size(){return this.tensors.length}read(e){if(this.closed_)throw new Error(`TensorArray ${this.name} has already been closed.`);if(e<0||e>=this.size())throw new Error(`Tried to read from index ${e}, but array size is: ${this.size()}`);const t=this.tensors[e];if(t.cleared)throw new Error(`TensorArray ${this.name}: Could not read index ${e} twice because it was cleared after a previous read (perhaps try setting clear_after_read = false?).`);return this.clearAfterRead&&(t.cleared=!0),t.read=!0,t.tensor}readMany(e){return e.map(t=>this.read(t))}write(e,t){if(this.closed_)throw new Error(`TensorArray ${this.name} has already been closed.`);if(e<0||!this.dynamicSize&&e>=this.maxSize)throw new Error(`Tried to write to index ${e}, but array is not resizeable and size is: ${this.maxSize}`);const s=this.tensors[e]||{};if(t.dtype!==this.dtype)throw new Error(`TensorArray ${this.name}: Could not write to TensorArray index ${e},
        ...
        ${s}, and tensor's shape is: ${t.shape}`);if(!this.dynamicSize&&e.length!==this.maxSize)throw new Error(`TensorArray's size is not equal to the size of lengths (${this.maxSize} vs. ${e.length}), and the TensorArray is not marked as dynamically resizeable`);const i=s===0?0:t.size/s,o=[];R(()=>{t=A(t,[1,s,i]);for(let p=0;p<e.length;++p){const l=[0,p===0?0:r[p-1],0],c=[1,e[p],i];o[p]=A(Z(t,l,c),this.elementShape)}return o});const u=[];for(let p=0;p<e.length;p++)u[p]=p;this.writeMany(u,o)}}/**
 * @license
 * Copyright 2020 Google LLC. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
